...
export interface AgentHermesRuntime {
  ...
  start(input: { input: string; instructions: string; sessionId: string; modelAlias: string }): Promise<string>;
...
function hardenedInstructions(run: LoadedRun, sources: KnowledgeSource[]): string {
  return `${run.version.instructions}\n\nAIHUB ENFORCED EXECUTION BOUNDARY\n` +
    `This is a single-turn, zero-tool run. Do not call, invent, or request tools, MCP servers, terminals, filesystems, networks, skills, or subagents. ` +
...
export class PrismaAgentProcessor {
  ...
  async process(payload: AgentRunJobPayload, jobId: string, workerId: string): Promise<object> {
    ...
    if (original.status === "CANCEL_REQUESTED" && !original.externalRunId) {
      await this.finish(original.id, "CANCELLED", "CANCELLED_BEFORE_START", "The run was cancelled before Hermes started.", workerId);
    ...
    try {
      ...
      if (!externalRunId) {
        ...
        externalRunId = await this.hermes.start({
          input: run.input,
          instructions: hardenedInstructions(run, sources),
          sessionId: run.id,
      ...
      while (Date.now() < deadline) {
        ...
        if (state.status === "completed") {
          ...
          await this.prisma.$transaction([
            this.prisma.agentRun.update({ where: { id: run.id }, data: { status: "COMPLETED", output: state.output, completedAt: new Date() } }),
  ...
  private async load(runId: string): Promise<LoadedRun | null> {
    return this.prisma.agentRun.findUnique({
      ...
      include: {
        ...
        version: { select: { instructions: true, modelAlias: true, maxTurns: true, timeoutSeconds: true, safeMode: true } },
      },
  ...
  private async finish(
    ...
    await this.prisma.$transaction([
      this.prisma.agentRun.updateMany({
        ...
        data: { status, failureCode, failureMessage: failureMessage.slice(0, 500), completedAt: new Date() },
      }),
      this.prisma.auditEvent.create({ data: {
