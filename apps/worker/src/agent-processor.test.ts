...
function runRecord(status = "QUEUED", externalRunId: string | null = null, jobId: string | null = null) {
  return {
    id: RUN_ID,
    status,
    jobId,
...
function database(record = runRecord(), enabled = true): AIHubPrismaClient {
  const state = { ...record };
  const agentRun = {
...
describe("PrismaAgentProcessor", () => {
  it("preflights the zero-tool boundary, scopes knowledge, and completes a run", async () => {
    ...
    const processor = new PrismaAgentProcessor(prisma, hermes, knowledge);
  ...
  it("finalizes cancellation without starting a remote run", async () => {
    ...
    const hermes = runtime();
    const processor = new PrismaAgentProcessor(prisma, hermes, { search: vi.fn() });
  ...
  it("denies approval requests and stops the Hermes run", async () => {
    ...
    const processor = new PrismaAgentProcessor(prisma, hermes, { search: vi.fn(async () => []) });
