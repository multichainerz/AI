...
export const connectionDefinitions: ConnectionDefinition[] = [
  ...
  },
  {
    ...
    secretFields: [
      ...
      { name: "mcpGatewayToken", label: "OrcaSynapse MCP gateway token", required: false },
    ],
    configurationFields: [
      ...
      { name: "toolsetsPath", label: "Toolsets path", type: "text", defaultValue: "/v1/toolsets", help: "OrcaSynapse refuses governed execution if any native Hermes toolset is enabled." },
      ...
      { name: "governedMcpUrl", label: "Governed MCP URL", type: "text", placeholder: "https://orcasynapse.internal/api/v1/mcp/", help: "Server-reachable OrcaSynapse MCP endpoint passed only through Hermes private run context." },
      { name: "governedToolsetName", label: "Governed toolset name", type: "text", defaultValue: "orcasynapse-governed-tools", help: "Exact Hermes toolset allowed when governed tools are active; all other enabled toolsets are denied." },
