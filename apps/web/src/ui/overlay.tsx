...
function OverlayChrome(props: OverlayProps & { drawer?: boolean }) {
  return (
    <DialogRoot open={props.open} onOpenChange={(open) => { if (!open) props.onClose(); }}>
      ...
      >
        <DialogHeader className="flex-row items-start justify-between gap-6 border-b border-border px-5 py-4 pr-5">
        ...
        {props.footer ? <footer className="flex items-center justify-end gap-2 border-t border-border px-5 py-3.5">{props.footer}</footer> : null}
      </DialogContent>
    </DialogRoot>
  );
}
