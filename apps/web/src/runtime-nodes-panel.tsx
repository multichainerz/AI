  ...
  return <div className="runtime-nodes-layout">
    ...
    {editorOpen && <div className="agent-editor-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditorOpen(false); }}><section className="setup-evidence-editor runtime-node-editor">
      ...
      {!invitation ? <form onSubmit={(event) => void issueInvitation(event)}>
        ...
        <ol><li><span>1</span><div><strong>Run the OrcaSynapse-hosted installer on VM2</strong><small>Review the downloaded script, then run it against this OrcaSynapse origin.</small><code>curl -fsS {invitation.bundle.controlPlaneUrl}/install/hermes-node.sh -o orcasynapse-node-install.sh{`\n`}sudo bash orcasynapse-node-install.sh --connect {invitation.bundle.controlPlaneUrl}</code></div></li><li><span>2</span><div><strong>Paste the claim when prompted</strong><small>The installer sends it in a redacted POST body, not in a URL or shell-history argument.</small><code>{invitation.bundle.token}</code><button className="secondary-button" type="button" onClick={() => void navigator.clipboard.writeText(invitation.bundle.token)}>Copy claim</button></div></li><li><span>3</span><div><strong>Let OrcaSynapse configure the runtime</strong><small>The claim resolves the approved Hermes image and addresses; enrollment then supplies the active model route and a node-scoped gateway key.</small><button className="secondary-button" type="button" onClick={downloadBundle}>Download JSON fallback</button></div></li></ol>
