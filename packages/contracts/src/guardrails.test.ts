...
describe("guardrail policy contracts", () => {
  it("accepts a bounded policy draft", () => {
    expect(createGuardrailPolicySchema.parse({
      ...
      liteLLMGuardrails: [...draft.liteLLMGuardrails],
      maxInputCharacters: draft.maxInputCharacters,
    })).toMatchObject({ slug: "chat-safety", maxInputCharacters: 12_000 });
  });

  it("rejects duplicate or unsafe LiteLLM guardrail names", () => {
    expect(createGuardrailPolicySchema.safeParse({ ...draft, liteLLMGuardrails: ["pii", "pii"] }).success).toBe(false);
    expect(createGuardrailPolicySchema.safeParse({ ...draft, liteLLMGuardrails: ["../../pii guard"] }).success).toBe(false);
  });
