 ...
 *
 * ai-v3.16.0 intentionally starts a new, incompatible schema generation.
  ...
  try {
    ...
    if (metadataTable.rows[0]?.table_name) {
      ...
      if (epoch.rows[0]?.epoch !== SCHEMA_EPOCH) {
        throw new Error(`Unsupported OrcaSynapse database epoch. ai-v3.16.0 requires a fresh installation (${SCHEMA_EPOCH}).`);
      }
    } else {
