...
VM2 runs exactly one agent runtime plane: the Hermes gateway/API server, installed natively from Hermes's own installer and supervised by systemd as `orcasynapse-hermes.service`. It durably owns Hermes-native sessions, Skills, `MEMORY.md`, and `USER.md` under `/var/lib/orcasynapse-hermes`. A root-owned, one-shot `orcasynapse-hermes-corpus` companion publishes an allowlisted signed mirror and applies signed mutations through Hermes-native APIs; it is not a memory runtime and never supplies model context. A sibling `orcasynapse-hermes-artifacts` companion publishes run deliverables saved under `artifacts/<sessionId>/` to the control plane — files at or under 4 MiB inline, larger ones as metadata — and reports files that later leave the disk; it reads and signs, and never deletes agent output. Destroying or cleanly re-enrolling VM2 without restoring Hermes state loses native session and memory continuity.
...
1. Run the displayed `remove-agentic-node.sh` command on VM2 and type `DESTROY` at its local terminal. It stops and removes the `orcasynapse-hermes` unit, corpus and artifact companions, Hermes installation, service account, node identity, managed policy, and the heartbeat, desired-state, corpus, and artifact timers. It preserves unrelated systemd units, Ubuntu packages, and external backups. Before any irreversible step it proves the unit is OrcaSynapse-managed - the unit file must carry `X-OrcaSynapse-Managed=true` or name the OrcaSynapse state root in `ReadWritePaths=` - and refuses to touch a Hermes someone else installed. The remover honors the same `ORCASYNAPSE_HERMES_STATE_ROOT` override the installer accepts and reports its own version in the completion panel.
