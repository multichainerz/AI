...
starts. The control plane PUTs bytes to **TCP 8643** (`orcasynapse-hermes-inbox.service`)
with the same Hermes API key as port 8642. Native `file` tools (`read_file`, `write_file`, `patch`, `search_files`)
