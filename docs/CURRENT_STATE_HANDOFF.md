...
- Current working branch: `fix/worker-run-durability` at `ai-v1.17.4`, one commit ahead of `main`, not yet pushed. See "Latest verified build state" for what it contains.
...
Since that capture, `ai-v1.17.4` on branch `fix/worker-run-durability` landed five worker defects found by review rather than by a failing test. `pnpm verify` passes there with 72 test files and 368 tests; worker coverage went from 14 to 23 tests. The fixes are:
