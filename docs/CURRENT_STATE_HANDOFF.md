...
- Baseline release: **ai-v1.28.0** (this file ships in that release commit; `git log -1` gives the hash). Releases are tagged starting at `ai-v1.25.0`.
- Baseline verification: `pnpm verify` passes — 81 test files, 622 tests, typecheck, production build, and `drizzle-kit check` all green. `pnpm verify:postgres` passes against a pgvector server.
...
8. Hermes and Supermemory run together on isolated VM2; small native Hermes files such as `MEMORY.md` and `USER.md` remain additive runtime state.
...
- PostgreSQL 17 with pgvector, Drizzle ORM 0.45 (migrations under `packages/database/drizzle/migrations`, currently 0000–0007, applied by the runtime migrator)
- Docker Compose for VM1; official Hermes container plus Supermemory Local systemd service on VM2
...
| `packages/runtime-clients` | Hermes and Supermemory server-side clients |
...
- **Agentic System**: one-time claims, Ed25519 identity, signed replay-protected enrollment and heartbeats (both-planes ONLINE rule, including the post-enroll trust proof), digest-pinned Hermes image, checksum-verified Supermemory (0.0.6 hard-blocked), resumable recovery journal, drain/suspend/revoke/remove lifecycle. Nodes show an expected DEGRADED window between enrollment and heartbeat startup.
...
## Known upstream caveat (VM2 agent memory only)

Supermemory `0.0.7-rc.2` (the pinned default for new nodes) ignores the `SUPERMEMORY_EMBEDDING_*` variables and falls back to `bge-base-en-v1.5`/768 (upstream issue #1336). The installer requests `Xenova/bge-m3`/1024, verifies what actually loaded, and warns prominently on mismatch. This affects **Hermes agent-memory quality only** — OrcaSynapse's document knowledge is embedded locally on VM1 and is unaffected. The separate `0.0.6` hard-block (packaging defects #1315/#1324) is a different rule; do not merge them. Multilingual-retrieval-sensitive deployments should verify the loaded model in the installer output before accepting a node.
...
After the WSL/LXD environment resumes, guest agents take time to initialize — a transient `LXD VM agent is not currently running` is not an OrcaSynapse regression. The lab's VM2 runs Supermemory `0.0.5` with verified BGE-M3/1024; do not reinstall it merely to obtain the release candidate (see the upstream caveat above).
