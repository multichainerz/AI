...
The API process scans every five seconds and drains a bounded batch of due connections through up to eight concurrent diagnostic lanes. PostgreSQL row locks with `SKIP LOCKED`, a unique claim token, and instance ownership prevent two AIHub instances from intentionally checking the same row. A claim becomes recoverable after two minutes if an instance terminates. Configuration edits and rollbacks invalidate health evidence and clear any active claim. Monitoring uses the same bounded, redirect-blocking, credential-aware adapters as a manual connection test; the configured cadence remains a target and saturation is visible as overdue `NOT_VERIFIED` health rather than being misrepresented as current.
