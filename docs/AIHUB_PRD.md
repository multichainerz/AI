...
Phase 9 uses upstream Hermes terminology precisely: Hermes calls a durable, separately configured `HERMES_HOME` a **Profile**, calls its portable package a **Profile Distribution**, and calls a short-lived child created by `delegate_task` a **subagent**. AIHub keeps versioned expert configurations as governed Profile Distributions, but the baseline does not pretend to create or mutate upstream Docker Profiles remotely. It injects the selected immutable behavior into each governed run on one enrolled runtime boundary. The Agent component will manage:
...
Hermes will continue to use runtime-local `state.db` SQLite storage for active session messages, tool-call history, context lineage, token accounting, and native resume. Supermemory does not replace this operational state or reduce its writes during execution. The baseline installer creates one protected `HERMES_HOME` for the enrolled runtime node; it does not create one upstream Profile directory per AIHub Profile Distribution. Hermes also keeps runtime-local `MEMORY.md` and `USER.md` active. They must not hold enterprise records or secrets, and multi-Profile use cannot claim filesystem-level memory isolation until a supported one-container-per-Profile deployment model is implemented and accepted. PostgreSQL owns AIHub conversations, sanitized run/event projections, governance, and audit; Supermemory owns only policy-approved durable enterprise knowledge and agent memory. AIHub will correlate these stores through documented APIs and identifiers, never by reading or synchronizing Hermes SQLite rows. `state.db` therefore requires explicit retention, protected local persistence, backup/restore, single-owner affinity, and honest loss behavior.
...
flowchart TB
    ...
    H --> HS["Runtime-local HERMES_HOME and state.db"]
...
- **Hermes runtime `state.db`:** Operational session history, context lineage, tool calls, token accounting, and native resume for the enrolled runtime's `HERMES_HOME`; never directly accessed by AIHub and never shared across hosts.
- **Hermes built-in memory:** Runtime-local `MEMORY.md` and `USER.md`; never an enterprise knowledge, secret, or compliance store. The baseline does not claim filesystem isolation between AIHub Profile Distributions.
