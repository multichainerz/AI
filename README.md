...
- one-time Hermes VM enrollment with a dashboard-generated bundle, node-local Ed25519 key, signed replay-protected heartbeats, automatic Hermes connection registration, bidirectional network evidence, revocation, and no retained SSH or Docker-socket trust;
...
For a release-bundle deployment on a clean Debian or Ubuntu server, run `sudo ./scripts/install-aihub.sh`. For a separated topology, open **Deployment > Hermes nodes**, issue the VM2 invitation, and run the dashboard-served installer on the prepared Hermes host. For local development, build first, then run `node scripts/generate-bootstrap.mjs` and `docker compose up -d --no-build`. See [`deploy/BOOTSTRAP.md`](deploy/BOOTSTRAP.md) and the [`Hermes node enrollment runbook`](docs/HERMES_NODE_ENROLLMENT_RUNBOOK.md) for recovery and production handling requirements.
...
AIHub Profile Distribution versions govern `SOUL.md` behavior, checksummed Skill references, model alias, limits, and policy as one SHA-256-addressed release. Standby and activation require the exact distribution digest as onboarding evidence. The Docker gateway remains a single isolated Hermes execution boundary; AIHub injects the selected immutable Profile behavior into each governed Runs API request instead of writing Profile files into Hermes or creating a second profile authority. The worker pins the distribution digest before queueing, so later dashboard edits cannot mutate an in-flight run.
