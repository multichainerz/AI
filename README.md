...
ai-v3.16.0 established the greenfield `hermes-native-v1` schema generation. ai-v3.17.0 added the Corpus companion; subsequent v3.17 patches hardened its privilege transition and ensure every mirrored Skill support tree has an observed parent. Existing v3.16 or v3.17 VM1 installations update in place; rerun the current generated VM2 installer with `--repair` to install the current companion. Databases from before v3.16 still require a clean installation.
